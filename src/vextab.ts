...
class VexTab {
  ...
  parseStaveOptions(options?: any[]): any {
    ...
    for (const option of options) {
      const error = (msg: string) => newError(option, msg);
  ...
  parseStaveText(text_line: any[]): void {
    ...
    const createNote = (text: string) => {
      ...
      try {
        ...
        throw newError(text_line[0], "Bad text or duration. Did you forget a comma?" + e);
      }
    };
  ...
  parse(code?: string): any {
    parser.parseError = (message: string, hash: any) => {
...
const newError = (object: any, msg: string) => {
