...
class Artist {
  ...
  render(renderer: any): void {
    ...
    const formatAndRender = (ctx: any, tab: any, score: any, text_notes: any[], customizations: any, options: any): any => {
      const tab_stave = tab != null ? tab.stave : undefined;
      const score_stave = score != null ? score.stave : undefined;
    ...
    const setBar = (stave: any, notes: any[]): void => {
  ...
  getNoteForABC(abc: any, string: any): [string, any, string | null] {
    const key = abc.key;
    const octave = string;
    let accidental: string | null = abc.accidental;
  ...
  setDuration(time: string, dot: boolean = false): void {
    ...
    const makeDuration = (time: string, dot: boolean): string => time + (dot ? "d" : "");
  ...
  closeBends(offset: number = 1): void {
    ...
    const start_index = this.bend_start_index + 1;
    const end_index = (tab_notes.length - 2) + offset;
    for (const tab_note of tab_notes.slice(start_index, end_index + 1)) {
  ...
  makeFingering(text: string): any[] | null {
    const getFingering = (text: string): RegExpMatchArray | null => text.match(/^\.fingering\/([^.]+)\./);
    ...
    const badFingering = (): Vex.RuntimeError => new Vex.RuntimeError("ArtistError", `Bad fingering: ${parts![1]}`);
  ...
  makeStroke(text: string): any | null {
    const getStrokeParts = (text: string): RegExpMatchArray | null => text.match(/^\.stroke\/([^.]+)\./);
  ...
  makeScoreArticulation(text: string): any | null {
    const getScoreArticulationParts = (text: string): RegExpMatchArray | null => text.match(/^\.(a[^\/]*)\/(t|b)[^.]*./)
  ...
  makeAnnotation(text: string): any | null {
    ...
    const makeIt = (text: string, just: any = default_vjust): any => {
  ...
  addAnnotations(annotations: string[]): void {
    ...
    if (stave.tab) {
      const start_index = tab_notes.length - annotations.length;
      for (let i = 0; i < annotations.length; i++) {
        const tab_note = tab_notes[start_index + i];
      ...
      const start_index = stave_notes.length - annotations.length;
      for (let i = 0; i < annotations.length; i++) {
        const note = stave_notes[start_index + i];
        if (!getScoreArticulationParts(annotations[i])) {
          const annotation = this.makeAnnotation(annotations[i]);
          if (annotation) {
  ...
  addArticulations(articulations: any[]): void {
    ...
    let has_bends = false;
    for (const valid_articulation of ["b", "s", "h", "p", "t", "T", "v", "V"]) {
      ...
      let prev_tab_note: any;
      let prev_indices: any;
      if (prev_index === -1) {
        prev_tab_note = null;
        prev_indices = null;
      } else {
        ...
        const prev_positions = (prev_tab_note as any).getPositions();
        for (let i = 0; i < prev_positions.length; i++) {
          const pos = prev_positions[i];
  ...
  addChord(chord: any[], chord_articulation?: any, chord_decorator?: string): void {
    ...
    const decorators: any[] = [];     // Decorators (vibratos, harmonics)
    ...
    for (const note of chord) {
      ...
      let new_octave: number | null = null;
      let accidental: string | null = null;
      ...
      const play_octave = parseInt(String(new_octave), 10) + this.current_octave_shift;
  ...
  addTextNote(text: string, position: number = 0, justification: string = "center", smooth: boolean = true, ignore_ticks: boolean = false): void {
    ...
    const lastVoice: any = _.last(voices);
    if (lastVoice) {
      lastVoice.push(note);
    }
  }
